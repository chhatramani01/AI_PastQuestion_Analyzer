...
# Default API key and model name (can be overridden by frontend)
...
def extract_questions_from_image(image, chapter, topic, model):
...
def analyze_questions(docx_path, model):
...
def index():
    if request.method == "POST":
        ...
        # Get API key and model name from headers (set by frontend)
        api_key = request.headers.get("X-API-Key", DEFAULT_API_KEY)
        model_name = request.headers.get("X-Model-Name", DEFAULT_MODEL_NAME)
        ...
        if file:
            ...
            all_questions = []
            for image in images:
                questions = extract_questions_from_image(image, chapter, topic, model)
                if questions:
            ...
            analyzed_questions = analyze_questions(docx_path, model)
